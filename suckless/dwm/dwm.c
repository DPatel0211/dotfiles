...
Window
XWindowAt(Display *dpy, int x, int y) {
	Window root_return, child_return;
	int dummy;
	unsigned int mask;

	if (XQueryPointer(dpy, root, &root_return, &child_return, &dummy, &dummy, &dummy, &dummy, &mask))
		return child_return;
	return None;
}
...
togglefloating(const Arg *arg)
{
	...
	//selmon->sel->x = selmon->sel->mon->mx + (selmon->sel->mon->mw - WIDTH(selmon->sel)) / 2;
	//selmon->sel->y = selmon->sel->mon->my + selmon->bar->bh + (selmon->sel->mon->mh - HEIGHT(selmon->sel) - selmon->bar->bh) / 2;
...
view(const Arg *arg)
{
	...
	/* NEW: Focus window under mouse after switching tags */
	int x, y;
	Window w;
	if (getrootptr(&x, &y) && (w = XWindowAt(dpy, x, y))) {
